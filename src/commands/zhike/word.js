  ...
  co(function* () {
    ...
    if (argv.word) {
      ...
      if (!word) {
        ...
        wordTable.push(['explanation', JSON.parse(word.explanation).map(w => `${w.pro} ${w.content}`).join('|')])
    ...
    while (true) {
      ...
      const answers = yield inquirer
        ...
        if (randomCorrectPlace === answers.selected) {
          ...
          correctWordTable.push(['explanation', JSON.parse(correctWord.explanation).map(w => `${w.pro} ${w.content}`).join('|')])
