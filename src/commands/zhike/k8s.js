...
const filterFuzzy = (list, keyword) => list.filter(item => new RegExp(keyword.split('').map(c => c.replace(/[.?*+^$[\]\\(){}|]/g, '\\$&')).join('.*'), 'i').test(item))
const filterContain = (list, keyword) => list.filter(item => item.indexOf(keyword) > -1)

exports.command = 'k8s <op> [keyword]'
exports.desc = 'k8s tools'
    ...
    switch (argv.op) {
      ...
      case 'logs':
        ...
        inquirer.prompt([
          ...
          const logsPods = answers.selectedPods.map(p => `${kubectl.generateCommand(['logs', '--tail=4', '-f', p])}`).join(' & ')
          shell.exec(`cat <(${logsPods})`, {
            shell: shell.which('bash').stdout
          })
        }).catch(function (e) {
